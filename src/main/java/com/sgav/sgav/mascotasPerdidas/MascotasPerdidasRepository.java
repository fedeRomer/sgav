...
public interface MascotasPerdidasRepository extends JpaRepository<MascotasPerdidas, Integer>, JpaSpecificationExecutor<MascotasPerdidas> {
    ...
    @Query(value = "SELECT * FROM mascotas_perdidas WHERE encontrado=false", nativeQuery = true)
    List<MascotasPerdidas> findAllMascotasPerdidasNoEncontradas(@Param("string") String string);
}